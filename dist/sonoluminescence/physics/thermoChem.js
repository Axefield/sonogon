...
exports.computeThermoDerivatives = computeThermoDerivatives;
exports.computeSpeciesDerivatives = computeSpeciesDerivatives;
exports.computeInternalEnergyDerivatives = computeInternalEnergyDerivatives;
 ...
 */
function vanDerWaalsPressure(n, // number of moles
V, // volume
T, // temperature
a, // van der Waals parameter a
b // van der Waals parameter b
) {
    const V_corrected = V - n * b;
    if (V_corrected <= 0) {
        // Volume too small, use ideal gas as fallback
        return (n * units_1.Constants.R_gas * T) / V;
    }
    const ideal_term = (n * units_1.Constants.R_gas * T) / V_corrected;
 ...
 */
function solveVanDerWaalsVolume(P, // Pressure [Pa]
T, // Temperature [K]
n, // Number of moles
a, // van der Waals parameter a
b, // van der Waals parameter b
tolerance = 1e-6, maxIterations = 100) {
    // Initial guess: ideal gas volume
    let V = (n * units_1.Constants.R_gas * T) / P;
    ...
    for (let iter = 0; iter < maxIterations; iter++) {
        ...
        const f = P_corr * V_corrected - n * units_1.Constants.R_gas * T;
        // f'(V) = d/dV[f(V)]
 ...
 * ∂P/∂T = n*R_gas/(V - n*b)
 */
function computeVanDerWaalsDerivative(state, params, volumeChangeRate, dTdt) {
    const { R } = state.hydro;
    ...
    const n_moles = n_total / units_1.Constants.N_A;
    ...
    // Partial derivatives
    const dP_dV = -(n_moles * units_1.Constants.R_gas * T) / (V_corrected * V_corrected)
        + (2.0 * vanDerWaals_a * n_moles * n_moles) / (volume * volume * volume);
    const dP_dT = (n_moles * units_1.Constants.R_gas) / V_corrected;
 ...
 */
function computeHeatCapacityCp(species, T) {
    // Base translational contribution (always 3/2 * R)
    const Cp_trans = (3.0 / 2.0) * units_1.Constants.R_gas;
    ...
    if (species === 'Ar' || species === 'Xe' || species === 'H' || species === 'O' || species === 'N') {
        ...
        Cp_rot = 3.0 * units_1.Constants.R_gas;
    }
    else {
        // Diatomic (O2, N2, OH): 2 rotational modes
        Cp_rot = 2.0 * units_1.Constants.R_gas;
    }
    ...
    if (species === 'H2O') {
        ...
        for (const theta_v of modes) {
            ...
            if (x > 0.01) {
                ...
                Cp_vib += units_1.Constants.R_gas * x * x * exp_x / ((exp_x - 1) * (exp_x - 1));
            }
        }
    }
 ...
 */
function computeHeatCapacityCv(species, T) {
    const Cp = computeHeatCapacityCp(species, T);
    return Cp - units_1.Constants.R_gas; // J/(mol·K)
}
/**
 * Compute mixture heat capacity (weighted average)
 */
function computeMixtureHeatCapacity(state, T, useDetailed) {
    if (!useDetailed) {
        // Simplified: average diatomic
        const Cp = (5.0 / 2.0) * units_1.Constants.R_gas;
        const Cv = Cp - units_1.Constants.R_gas;
        return { Cp, Cv };
    }
    ...
    let totalCv = 0;
    let totalMoles = 0;
    for (const [speciesId, n] of Object.entries(species)) {
        const n_moles = (n * volume) / units_1.Constants.N_A;
        if (n_moles > 0) {
 ...
 */
function computeTemperatureDependentGamma(T, gamma0 = 1.4) {
 ...
 */
function computeThermoDerivatives(state, params) {
    const { R, Rdot } = state.hydro;
    const { Pg, T } = state.gas;
    const { gamma: gamma0, heatLossCoeff, useNonIdealGas = false, vanDerWaals_a = 0.136, // Argon: 0.136 Pa·m⁶/mol²
    vanDerWaals_b = 3.2e-5, // Argon: 3.2×10⁻⁵ m³/mol
    useTemperatureDependentGamma = false, gammaFunction, } = params;
    ...
    if (useNonIdealGas && vanDerWaals_a > 0 && vanDerWaals_b > 0) {
        ...
        if (params.useIterativeVanDerWaals) {
            ...
            dPgdt = computeVanDerWaalsDerivative(state, params, volumeChangeRate, dTdt);
        }
        else {
 ...
 */
function computeHeatLossRate(state, params, R) {
    const { T } = state.gas;
    ...
    if (params.useDetailedHeatTransfer) {
        ...
        if (params.includeRadiation) {
            ...
            const sigma_SB = units_1.Constants.sigma_SB;
        ...
        const heatLossRate = totalHeatLoss / Math.max(heatCapacity, units_1.Constants.k_B * 1e20);
        return heatLossRate;
    }
    else {
        // Simplified heat loss (original)
        const heatLossRate = -params.heatLossCoeff * (T - T_ambient) * surfaceArea / (units_1.Constants.k_B * 1e20);
        return heatLossRate;
    }
}
/**
 ...
 */
function computeSpeciesDerivatives(state, params) {
    const { R, Rdot } = state.hydro;
    const R_safe = Math.max(R, 1e-10);
    const volume = (4.0 / 3.0) * Math.PI * R_safe * R_safe * R_safe;
    ...
    if (params?.useSpeciesDiffusion && params.diffusionCoefficients) {
        ...
        for (const species of Object.keys(dNdt)) {
            ...
            if (D && D > 0) {
                ...
                const C_liquid = (liquidConcs[species] || 0) * units_1.Constants.N_A; // Convert to number density
 ...
 */
function computeRelaxationTime(tau0, T, T_ref = 1000, alpha = 0.5) {
    if (T <= 0)
        return tau0;
 ...
 */
function computeLandauTellerRelaxationTime(tau0, T, T_vib = 3000) {
    if (T <= 0)
        return tau0;
 ...
 */
function computeInternalEnergyDerivatives(state, params) {
    const { R, Rdot } = state.hydro;
    const { T } = state.gas;
    const R_safe = Math.max(R, 1e-10);
    ...
    const E_trans_eq = (3.0 / 2.0) * units_1.Constants.k_B * T * n_total;
    // Rotational: k_B * T per rotational mode (assuming diatomic, 2 modes)
    const E_rot_eq = 2.0 * units_1.Constants.k_B * T * n_total;
    ...
    const E_vib_eq = units_1.Constants.k_B * T * n_total * vib_factor;
    // Electronic: excited states (Boltzmann distribution)
    // E_elec ≈ k_B * T * exp(-E_excited / (k_B * T))
    const E_excited = 10 * units_1.Constants.e; // ~10 eV excited state
    const elec_factor = Math.exp(-E_excited / (units_1.Constants.k_B * T));
    const E_elec_eq = units_1.Constants.k_B * T * n_total * elec_factor * 0.1;
    ...
    const dE_trans_dt = -E_trans * volumeChangeRate + // compression work
        (E_trans_eq - E_trans) / tau_trans; // relaxation
    const dE_rot_dt = -E_rot * volumeChangeRate +
        (E_rot_eq - E_rot) / tau_rot;
    const dE_vib_dt = -E_vib * volumeChangeRate +
        (E_vib_eq - E_vib) / tau_vib;
    const dE_elec_dt = -E_elec * volumeChangeRate +
...
//# sourceMappingURL=thermoChem.js.map