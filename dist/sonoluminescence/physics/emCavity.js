 ...
 */
function computeModeFrequency(omega0, R, R0, omega_p, n_effective, // Refractive index (if available)
opticalFrequency // Optical frequency for plasma refractive index
) {
 ...
 */
function computeRefractiveIndexTimeDerivative(state, statePrev, dt, opticalFrequency) {
    ...
    const totalNeutralDensityPrev = Object.values(numberDensityPrev).reduce((sum, n) => sum + n, 0);
    const n_neutral_prev = 1.0 + alpha * totalNeutralDensityPrev;
    const omega_p_prev = units_1.Calculations.plasmaFrequency(ne_prev);
 ...
 */
function computeEmDerivatives(state, params, statePrev, // Previous state for computing time derivatives
dt // Time step for computing time derivatives
) {
    ...
    const dModes = Array.from({ length: nModes }, (_, k) => {
        ...
        const omega_k = computeModeFrequency(omega0, R, R0, omega_p, n_effective, // Include refractive index if enabled
        opticalFrequency);
