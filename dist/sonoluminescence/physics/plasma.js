...
exports.computePlasmaDerivatives = computePlasmaDerivatives;
 ...
 */
function computeSahaEquilibrium(T, n_neutral, ionizationPotential) {
    // Statistical weights (simplified: g_i/g_0 ≈ 1 for ground states)
    const g_ratio = 1.0;
    // Prefactor: (2*π*m_e*k_B*T / h²)^(3/2)
    const prefactor = Math.pow((2.0 * Math.PI * units_1.Constants.m_e * units_1.Constants.k_B * T) / (units_1.Constants.h * units_1.Constants.h), 1.5);
    // Exponential: exp(-I / (k_B*T))
    const exponent = Math.exp(-ionizationPotential / (units_1.Constants.k_B * T));
 ...
 */
function computePlasmaDerivatives(state, params) {
    ...
    if (params.useDetailedCollisions) {
        ...
        const v_th_e = Math.sqrt((units_1.Constants.k_B * Te) / units_1.Constants.m_e); // Electron thermal velocity
        const v_th_i = Math.sqrt((units_1.Constants.k_B * T) / (units_1.Constants.m_p * 40)); // Ion thermal velocity (Ar)
    ...
    const dn_edt = -ne * volumeChangeRate + // compression/expansion
    ...
    if (params.useDetailedCollisions && params.collisionModel === 'temperature-dependent') {
        ...
        if (params.useTemperatureDependentCrossSections) {
            const E_th = 1.0 * units_1.Constants.e; // Threshold energy ~1 eV
            const E_e = units_1.Constants.k_B * Te;
        ...
        const mass_ratio = units_1.Constants.m_e / (units_1.Constants.m_p * 40); // m_e / m_Ar
        ...
        const mass_ratio = units_1.Constants.m_e / (units_1.Constants.m_p * 40);
        collisionalCooling = -(2.0 * mass_ratio) * nu_coll * (Te - T);
    }
    else {
    ...
    const bremsstrahlungPower = units_1.Calculations.bremsstrahlungPowerDensity(ne, Te);
    const bremsstrahlungCooling = -bremsstrahlungPower / (ne * units_1.Constants.k_B * 1e6); // Simplified normalization
    ...
    if (params.includeRadiationLoss) {
        ...
        radiationCooling = -radiationPower / (ne * units_1.Constants.k_B * 1e6);
    }
