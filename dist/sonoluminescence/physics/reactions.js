...
exports.computeReactionDerivatives = computeReactionDerivatives;
exports.computeSpeciesReactionRates = computeSpeciesReactionRates;
...
function arrheniusRate(A, Ea, T) {
    if (T <= 0)
        return 0;
    return A * Math.exp(-Ea / (units_1.Constants.R_gas * T));
}
/**
 ...
 */
function lindemannRate(k0, // Low-pressure limit [m³/(mol·s)]
kInf, // High-pressure limit [1/s]
M, // Third-body concentration [mol/m³]
Fc = 0.6 // Broadening factor (Troe parameter)
) {
    if (M <= 0)
        return kInf; // No third body, use high-pressure limit
 ...
 */
function computeReactionDerivatives(state, params) {
    const { T } = state.gas;
    ...
    if (nReactions > 0) {
        ...
        if (params.usePressureDependentRates && params.reaction0_k0 && params.reaction0_kInf) {
            ...
            const M = (n_total * volume) / units_1.Constants.N_A; // Third-body concentration [mol/m³]
            k_f0 = lindemannRate(params.reaction0_k0, params.reaction0_kInf || k_f0, M, params.reaction0_Fc || 0.6);
        }
 ...
 */
function computeSpeciesReactionRates(state, params) {
    const { T } = state.gas;
    const { numberDensity } = state.species;
    const { xi } = state.reactions;
