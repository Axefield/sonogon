...
export interface HydroParams {
  ...
  // Shape-radial coupling
  enableShapeRadialCoupling?: boolean; // Couple shape oscillations to radial dynamics
  shapeCouplingCoefficient?: number; // Coupling strength (default: 0.1)
}
...
export function computeHydroDerivatives(
  ...
  const { Pg } = state.gas;
  const { 
    rho, mu, sigma, Pv, P0, c = 1482, 
    useKellerMiksis = false,
    enableShapeRadialCoupling = false,
    shapeCouplingCoefficient = 0.1,
  } = params;
