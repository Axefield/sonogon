...
export type EventFunction = (t: number, x: Float64Array) => number; // Returns 0 when event occurs

export interface IntegratorOptions {
  dt: number; // Initial time step
  tMax: number; // Maximum time
  dtMin?: number; // Minimum time step (default: 1e-15)
  dtMax?: number; // Maximum time step (default: 1e-6)
  tolerance?: number; // Error tolerance for adaptive methods (default: 1e-6)
  adaptive?: boolean; // Use adaptive step size (default: true)
  maxSteps?: number; // Maximum number of steps (default: 1e6)
}

export interface IntegrationResult {
  t: number[];
  x: Float64Array[];
  events?: Array<{ time: number; state: Float64Array; eventIndex: number }>;
  stats: {
    steps: number;
 ...
 */
export function integrateDormandPrince(
  rhs: RhsFunction,
  x0: Float64Array,
  options: IntegratorOptions
): IntegrationResult {
...
export function integrateAdaptive(
  ...
  if (useAdaptive) {
    ...
    if (events && events.length > 0) {
      const detectedEvents: Array<{ time: number; state: Float64Array; eventIndex: number }> = [];
...
function scaleVec(a: Float64Array, s: number): Float64Array {
  const out = new Float64Array(a.length);
