...
export function createEquilibriumState(
  ...
  const n_total = Calculations.numberDensityFromPressure(Pg_eq, T_eq);
  ...
  return {
    ...
    internalEnergy: {
      ...
      translational: (3.0 / 2.0) * Constants.k_B * T_eq * n_total * (4.0 / 3.0) * Math.PI * R_equilibrium * R_equilibrium * R_equilibrium,
      rotational: 2.0 * Constants.k_B * T_eq * n_total * (4.0 / 3.0) * Math.PI * R_equilibrium * R_equilibrium * R_equilibrium,
      vibrational: Constants.k_B * T_eq * n_total * (4.0 / 3.0) * Math.PI * R_equilibrium * R_equilibrium * R_equilibrium,
      electronic: 0.01 * Constants.k_B * T_eq * n_total * (4.0 / 3.0) * Math.PI * R_equilibrium * R_equilibrium * R_equilibrium,
    },
    em: {
      modes: [
        {
...
export function createCollapseState(
  params: SonoluminescenceParams,
  R_min_start: number,
  Rdot_collapse: number = -100 // m/s, negative for collapse
): BubbleFullState {
  const equilibrium = createEquilibriumState(params, params.thermo.R0);
  ...
  const ne_estimate = Calculations.numberDensityFromPressure(Pg_compressed, T_compressed) * 0.01; // Rough estimate
 ...
 */
export function validateInitialState(state: BubbleFullState): {
  valid: boolean;
  errors: string[];
} {
  const errors: string[] = [];
  ...
  const totalSpecies = Object.values(state.species.numberDensity).reduce(
    (sum, n) => sum + n,
    0
  );
  if (totalSpecies <= 0) {
